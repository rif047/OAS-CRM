...
export default function SurveyorDashboard() {
    ...
    const canAccess = userType === "Surveyor" || userType === "Admin" || userType === "Management";
    ...
    const loadDashboard = async () => {
        ...
        setError("");
        try {
            const res = await axios.get(`${import.meta.env.VITE_SERVER_URL}/api/leads/dashboard/surveyor`);
            setData({
                summary: res.data?.summary || {},
                statusBreakdown: Array.isArray(res.data?.statusBreakdown) ? res.data.statusBreakdown : [],
                upcomingSchedule: Array.isArray(res.data?.upcomingSchedule) ? res.data.upcomingSchedule : [],
                dateContext: res.data?.dateContext || { today: "", month: "" },
                surveyor: res.data?.surveyor || null,
            });
            setLastUpdated(new Date());
        } catch {
            setError("Unable to load surveyor dashboard data.");
        } finally {
            setLoading(false);
        }
    };
        ...
        return (
            <Layout>
                <div className="rounded-xl border border-red-100 bg-red-50 p-5 text-red-800">
                    Surveyor dashboard access is restricted.
                </div>
            </Layout>
        );
    }
            ...
            <section className="space-y-4 pb-2">
                <div className="overflow-hidden rounded-xl border border-[#48506a] bg-linear-to-r from-[#4c5165] to-[#2f3548] shadow-sm">
                    <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-4 md:px-5">
                        <div>
                            <h1 className="text-xl font-bold text-white">Surveyor Dashboard</h1>
                            <p className="text-sm text-gray-200">
                                {data.surveyor || user?.name || "Surveyor"} - assigned surveys, deadlines and progress.
                            </p>
                        </div>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-5">
                    ...
                    <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                        <div className="mb-2 flex items-center gap-2 text-gray-700">
                            ...
                            <p className="text-sm font-semibold">In Survey Now</p>
                        </div>
                        <p className="text-3xl font-bold text-gray-900">{data.summary.inSurveyNow || 0}</p>
                    </div>
                ...
                <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
                    ...
                    <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
                        <div className="border-b border-gray-100 px-4 py-3">
                            <h2 className="text-base font-bold text-gray-900">Upcoming Survey Schedule</h2>
                            <p className="text-xs text-gray-500">Nearest assigned survey dates.</p>
                        </div>
                        <div className="max-h-[352px] overflow-auto px-4 py-2">
                            {(data.upcomingSchedule || []).length === 0 ? (
                                <p className="py-6 text-sm text-gray-500">No upcoming survey schedule.</p>
                            ) : (
                                data.upcomingSchedule.map((item) => (
                                    <div key={item._id} className="flex items-center justify-between border-b border-gray-100 py-3 last:border-b-0">
                                        ...
                                        <div className="text-right">
                                            <p className="text-sm font-semibold text-gray-700">{formatLondonDateShort(parseDate(item.survey_date))}</p>
